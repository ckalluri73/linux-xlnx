...
static int xhdmirxss_hdcp1x_keymgmt_init_tables(struct xhdmirx_state *state)
{
	...
	ret = xhdmirxss_hdcp1x_keymgmt_get_num_of_tables_rows(state, &num_tables,
							      &num_rows_per_table);
	if (ret) {
		dev_err(state->xvip.dev, "Failed to get number of tables\n");
		return ret;
	}
						...
						u32 key_table_size)
{
	...
	for (row_id = 0; row_id < (key_table_size / sizeof(u64)); row_id++) {
		...
		if (data != key_table->data_u64[row_id]) {
			dev_info(state->xvip.dev, "Key Verification Failed\n");
			ret = -EIO;
		}
	}

	dev_info(state->xvip.dev, "### Key Verification Done ###\n");
...
static int xhdmirxss_hdcp1x_keymgmt_set_key(struct xhdmirx_state *state)
{
	...
	if (ret) {
		dev_err(state->xvip.dev, "Failed to reset HDCP1X keymgmt core\n");
		return ret;
	}

	ret = xhdmirxss_hdcp1x_keymgmt_init_tables(state);
	if (ret) {
		dev_err(state->xvip.dev, "Failed to initialize HDCP1X keymgmt table ID's\n");
		goto error;
	}

	ret = xhdmirxss_hdcp1x_keymgmt_disable(state);
	if (ret) {
		dev_err(state->xvip.dev, "Failed to disable HDCP1X keymgmt IP core\n");
		goto error;
	}
	...
	ret = xhdmirxss_hdcp1x_keymgmt_load_keys(state, &key_table,
						 XHDCP1X_KEYS_SIZE);
	if (ret) {
		dev_err(state->xvip.dev, "Failed to Load HDCP1X keys\n");
		goto error;
	}
	...
	ret = xhdmirxss_hdcp1x_keymgmt_verify_keys(state, &key_table,
						   XHDCP1X_KEYS_SIZE);
	if (ret) {
		dev_err(state->xvip.dev, "Failed to Verify HDCP1X keys\n");
		goto error;
	}

	ret = xhdmirxss_hdcp1x_keymgmt_enable(state);
	if (ret) {
		dev_err(state->xvip.dev, "Failed to enable HDCP1X keymanagement IP\n");
		goto error;
	}
				      ...
				      struct xhdmirxss_hdcp1x_keys_ioctl *hdcp_keys)
{
	...
	if (ret < 0) {
		dev_err(xhdmirxss->xvip.dev, " ERROR while loading keys\n");
		return ret;
	}
...
{
	return container_of(sd, struct xhdmirx_state, xvip.subdev);
}
...
static long xhdmirxss_ioctl(struct v4l2_subdev *sd, u32 cmd, void *arg)
{
	...
	if (!xhdmirxss->hdcp_enable) {
		dev_err(xhdmirxss->xvip.dev, "HDCP is not enabled in the system");
		return -ENODEV;
	}

	if (xhdmirxss->hdcp1x_key_available) {
		dev_info(xhdmirxss->xvip.dev, "HDCP1X keys are already loaded");
		return -EPERM;
	}
...
static int xhdmirx_parse_of(struct xhdmirx_state *xhdmi)
{
	...
	if (xhdmi->hdcp_enable) {
		...
		if (!xhdmi->is_hdcp1x_enabled)
			dev_info(xhdmi->xvip.dev, "HDMI:HDCP 1.4 is not enabled\n");
		else
			dev_info(xhdmi->xvip.dev, "HDMI:HDCP 1.4 is enabled\n");
	}
...
static void xhdmirx_hdcp1x_notification_handler(void *ref, u32 notification)
{
	...
	case XHDCP1X_RX_NOTIFY_AUTHENTICATED:
		dev_info(xhdmirxss->xvip.dev, "HDCP1X Rx Authenticated\n");
		break;
	case XHDCP1X_RX_NOTIFY_UN_AUTHENTICATED:
		dev_info(xhdmirxss->xvip.dev, "HDCP1X Rx Un-Authenticated\n");
		break;
	default:
		dev_info(xhdmirxss->xvip.dev, "Undefined HDCP Notification\n");
		break;
	}
}
...
{
	xhdmirxss->hdcp1x = xhdcp1x_rx_init(xhdmirxss->xvip.dev, xhdmirxss,
					    xhdmirxss->xvip.iomem + XHDMIRX_HDCP1X_REG_OFFSET,
					    0, XHDCP1X_HDMI);

	if (IS_ERR(xhdmirxss->hdcp1x)) {
		dev_err(xhdmirxss->xvip.dev, "failed to initialize HDCP1X\n");
		return PTR_ERR(xhdmirxss->hdcp1x);
	}

	xhdmirxss->hdcp1x_key = devm_kzalloc(xhdmirxss->xvip.dev, XHDCP1X_KEYS_SIZE,
					     GFP_KERNEL);
...
static int xhdmirx_hdcp_init(struct xhdmirx_state *xhdmi)
{
	...
	xhdmi->hdcp1x_keymgmt_base =
		syscon_regmap_lookup_by_phandle(xhdmi->xvip.dev->of_node, "xlnx,hdcp1x_keymgmt");
	if (IS_ERR(xhdmi->hdcp1x_keymgmt_base)) {
		dev_err(xhdmi->xvip.dev, "couldn't map HDCP1X Keymgmt registers\n");
		return -ENODEV;
	}

	ret = xhdmirx_register_hdcp1x_dev(xhdmi);
	if (ret < 0) {
		dev_err(xhdmi->xvip.dev, "HDMI RX HDCP1X init failed\n");
		return -EINVAL;
	}

	irq = irq_of_parse_and_map(xhdmi->xvip.dev->of_node, 1);
	ret = devm_request_irq(xhdmi->xvip.dev, irq,
			       xhdmirxss_hdcp1x_irq_handler,
			       IRQF_SHARED, "hdmirxss_hdcp1x", xhdmi);
	if (ret) {
		dev_err(xhdmi->xvip.dev, "ERR: HDCP1X interrupt registration failed!\n");
		return -EINVAL;
	}
...
static int xhdmirx_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		dev_err(xhdmi->xvip.dev, "failed to initialize HDCP\n");
		goto hdcp_error;
	}
